...
extern void stkAddKeyEvent(int key_code, int action, int meta_state, int scan_code, int unichar);
...
int
Android_OnPadDown(int device_id, int keycode, int meta_state, int scan_code, int unichar, int repeat_count)
{
    ...
    else if (button >= 0)
    {
        ...
        stkAddKeyEvent(keycode, AKEY_EVENT_ACTION_DOWN, meta_state, scan_code, unichar);
        return 0;
    }
...
int
Android_OnPadUp(int device_id, int keycode, int meta_state, int scan_code, int unichar)
{
    SDL_joylist_item *item = JoystickByDeviceId(device_id);
    int button = keycode_to_SDL(keycode);
    if (item && item->joystick)
    ...
    {
        stkAddKeyEvent(keycode, AKEY_EVENT_ACTION_UP, meta_state, scan_code, unichar);
        return 0;
    }
